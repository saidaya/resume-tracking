...
try:
    with open('./patterns/skill_db_relax_20.json') as json_file:
        SKILL_DB = json.load(json_file)
except:
    SKILL_DB = bucket.fetch_remote("SKILL_DB")
    # dump data
    with open('./patterns/skill_db_relax_20.json', 'w') as fp:
        json.dump(SKILL_DB, fp)

# load token distribution dict
try:
    with open('./patterns/token_dist.json') as json_file:
        TOKEN_DIST = json.load(json_file)
except:
    TOKEN_DIST = bucket.fetch_remote("TOKEN_DIST")
    # dump data
    with open('./patterns/token_dist.json', 'w') as fp:
